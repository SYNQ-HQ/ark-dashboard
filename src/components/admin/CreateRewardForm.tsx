    ...
    return (
        <form ref={formRef} action={handleSubmit} className="space-y-4">
            <div>
                ...
                <input name="name" required className="w-full px-3 py-2 rounded-lg border border-border bg-background" placeholder="e.g. $10 Amazon Card" />
            </div>
            ...
            <div>
                ...
                <input name="cost" type="number" required defaultValue={1000} className="w-full px-3 py-2 rounded-lg border border-border bg-background" />
            </div>

            <div>
                <label className="block text-sm font-medium mb-1">Initial Stock (Optional)</label>
                <input name="stock" type="number" className="w-full px-3 py-2 rounded-lg border border-border bg-background" placeholder="Leave empty for infinite" />
            </div>
            ...
            <button disabled={uploading} type="submit" className="w-full py-2 bg-primary text-primary-foreground font-bold rounded-lg hover:brightness-110 transition-all mt-4 disabled:opacity-50">
                {uploading ? 'Uploading...' : 'Create Reward'}
            </button>
        </form>
    );
}
